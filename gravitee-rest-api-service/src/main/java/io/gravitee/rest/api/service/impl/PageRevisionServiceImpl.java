...
public class PageRevisionServiceImpl extends TransactionalService implements PageRevisionService {
    ...
    @Override
    public Optional<PageRevisionEntity> findById(String revisionId) {
        logger.debug("get page revision {}", revisionId);
        try {
            return pageRevisionRepository.findById(revisionId).map(this::convert);
        } catch (TechnicalException e) {
            logger.warn("An error occurs while trying to get the page revision {}", revisionId, e);
    ...
    private PageRevision convert(Page page) {
        ...
        String revId = revision.getPageId() + ":" + revision.getRevision();
