...
public class PageService_UpdateTest {
    ...
    private void shouldUnpublishPage(PlanStatus closed) throws TechnicalException {
        ...
        when(plan.getStatus()).thenReturn(closed);
    ...
    private void shouldNotUnpublishPage(PlanStatus deprecated) throws TechnicalException {
        final String API_ID = "API_ID_TEST";
        Page unpublishedPage = new Page();
        unpublishedPage.setId(PAGE_ID);
        unpublishedPage.setOrder(1);
        ...
        PlanEntity plan = mock(PlanEntity.class);
        when(plan.getGeneralConditions()).thenReturn(PAGE_ID);
        when(plan.getStatus()).thenReturn(deprecated);
        when(planService.findByApi(API_ID)).thenReturn(Sets.newSet(plan));
